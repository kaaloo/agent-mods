...
| L-MODEL | `phase.model` pattern validation | Closed | Per-phase model overrides were removed. The run captures `ctx.model.id`, tries that parent-conversation model first, and retries once with Auto if the preferred model cannot launch. |

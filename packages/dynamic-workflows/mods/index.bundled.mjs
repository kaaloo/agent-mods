...
function serializeWorkflowMarkdown(workflow, body = "") {
...
function loadWorkflowFromMarkdown(text) {
...
function recordAgentCompleteLocked(runId, phaseId, agentId, output) {
  const run = loadRun(runId);
  if (!run)
    return null;
  if (run.status !== "running")
...
function recordBarrierCompleteLocked(runId, phaseId, output) {
  const run = loadRun(runId);
  if (!run)
    return null;
  if (run.status !== "running")
...
function dispatchBarrierLocked(run, phase) {
...
function completeRunLocked(run, result) {
  run.status = "completed";
  run.currentPhaseId = null;
  const fileResult = readRunResult(run.runId, run.agentId);
  ...
  if (letta.capabilities?.tools && letta.tools) {
    ...
    disposers.push(letta.tools.register({
      ...
      run(ctx) {
        ...
        const filtered = !filterText ? safeAll : safeAll.filter((e) => e.name.toLowerCase().includes(filterText) || e.description.toLowerCase().includes(filterText));

...
function isOkfConcept(fm) {
  return typeof fm.type === "string" && fm.type.length > 0;
}
function asOkfFrontmatter(fm) {
  return fm;
}
var VALID_STATUSES = new Set(["draft", "stable", "deprecated"]);
function validateOkfFrontmatter(fm, isWrite) {
  const issues = [];
...
function validateSource(s, issues) {
...
function generateProvenance(agentId) {
  return {
    by: agentId ?? "unknown",
...
function activate(letta) {
  ...
  if (letta.capabilities?.permissions && letta.permissions) {
    disposers.push(letta.permissions.register({
      id: "okf-trust",
      description: "Enforce OKF v0.2 trust signals (provenance, verification, freshness, lifecycle) on MemFS memory writes.",
      check(event) {
        ...
        if (!extraction)
          return;
        if (!isOkfConcept(extraction.parsed))
          return;
...
${errors2.map((e) => `  - ${e.field}: ${e.message}`).join(`
`)}`
          };
        }
...
`)}`
  ...
  if (letta.capabilities?.events?.tools) {
    disposers.push(letta.events.on("tool_end", (event, _ctx) => {
